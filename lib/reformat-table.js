...
function reformat(str, delimiter) {
  ...
  hasHeaderSeparator = (table[1].join('').indexOf('-') >= 0);
  ...
  return table.map(function(row, row_index) {
    return (delimiter === '\t' ? '' : delimiter) + row.map(function(cell, column_index) {
      ...
      return (delimiter === '\t' ? '' : ' ') + utils.padStringWithAlignment(cell, column_length, alignments[column_index]) + (delimiter === '\t' ? '' : ' ');
