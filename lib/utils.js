...
function splitStringToTable(str, delimiter) {
  if (undefined === delimiter) {
    delimiter = '|';
  }

  let trimLeftRegexp = new RegExp('^[' + delimiter + '\s]+'),
    trimRightRegexp = new RegExp('[' + delimiter + '\s]+$');
  ...
  return trim(String(str)).split('\n').map(function (row) {
    row = row.replace(trimLeftRegexp, '');
    row = row.replace(trimRightRegexp, '');
    return row.split(delimiter).map(trim);
  });
}
